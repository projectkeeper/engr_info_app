...
<!--
  ...
  <table class="auto_position">
    ...
    <tr>
      ...
      <td>
        <div class="iptxt">
          <input type="text" name="item_name" value = "{{old('item_name')}}" placeholder="ロール"></input>
        </div>
      </td>
    </tr>
  </table>
...
@isset($master_info)
      ...
      @foreach($master_info as $data)
        ...
        @endphp
          <tr>
            ...
            <td class="{{$class_item}}" >
              <div class="btn-flat-border">
                <a href="javascript:button_press('','','{{$data['id']}}','open_edit_role')">更新</a><br>
              </div>
            </td>
          <tr>
      @endforeach

      @for ($i = 0; $i < config('const.master_line_num_role'); $i++)
        @php
          $switch = ++$switch % 2;
          $class_item ="item_value_3_".$switch;
        @endphp
        <tr>
          <td class="{{$class_item}}">
            <div class="iptxt">
              <input type="text" name="item_name_{{$i}}" value = "" placeholder="ロール名"/>
            </div>
          </td>
          <td class="{{$class_item}}">
            <div class="iptxt">
              <input type="text" name="item_value_{{$i}}" value = "" placeholder="ロール値"/>
            </div>
          </td>
          <td class="{{$class_item}}">
            <div class="iptxt">
    ...
    <input type="hidden" name="line_num" value= "{{config('const.master_line_num_role')}}" />
    ...
    <div class="box3">
      <table class="auto_position">
        <tr>
          ...
          <td>
            <div class="btn-flat-border">
              <a href="javascript:button_press('','','','check_new_role')">新規登録</a><br>
            </div>
          </td>
        </tr>
      </table>
